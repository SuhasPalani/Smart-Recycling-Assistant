...
def main():
    ...
    st.write(
        "Upload an image of an item to get recycling recommendations and nearby disposal locations."
    )
    ...
    if st.session_state.analysis_complete and st.session_state.uploaded_image:
        ...
        with col2:
            ...
            st.success(
                f"**Analysis complete in {st.session_state.process_time:.2f} seconds**"
            )

            # Create a styled box for the detected object
            st.markdown(
                f"""
            <div style="padding:10px; border-radius:5px; background-color:rgba(33, 150, 243, 0.2); border-left:5px solid #2196F3;">
                <h3 style="margin:0; color:white;">Detected: {st.session_state.recycling_type.upper()}</h3>
                <p style="color:white;">Matched items: {", ".join(st.session_state.matched_labels)}</p>
        ...
        if use_custom_location:
            ...
            st.info(
                "Using default location (Chicago, IL). Check the box above to enter a custom location."
            )
        ...
        if st.button("Find Nearby Disposal Locations"):
            with st.spinner("Finding locations..."):
                locations = get_nearby_disposal_locations(
                    st.session_state.recycling_type, latitude, longitude
                )

            if locations:
                st.success(
                    f"Found {len(locations)} nearby locations for {st.session_state.recycling_type} disposal"
                )
                ...
                st.warning(
                    "No nearby disposal locations found. Try expanding your search area."
                )
